...
The Northwestern Bat Hub for Population Monitoring and Research (Northwest Bat Hub) has collaborated with governmental partners to collect a robust summer acoustic monitoring dataset for the 15 bat species found in the Pacific Northwest, beginning in 2016. Here, we demonstrate how we can leverage these data to create high-resolution (\~1km^2^) range maps covering Oregon, Washington, and Idaho using the popular Maximum Entropy (MaxEnt) approach in 'R'. Results show well-calibrated models that are consistent with previously published literature in Oregon and Washington, and serve as one of the first attempts at creating predictive maps for all common northwestern species in the state of Idaho. While MaxEnt analyses are not commonly published in isolation, this exercise proves as useful tool to glean valuable information regarding habitat suitability for the bats of the Pacific Northwest and can aid in conservation eplanning when used in conjunction with the domain expertise of regional biologists.

# Background

The Northwestern Hub for Bat Population Research and Monitoring (Northwest Bat Hub) began efforts at annual summer occupancy surveys for the 15 bat species found in the Pacific Northwest in 2016 as a regional member of the North American Bat Monitoring Program (NABat). This effort has led to the collection of a robust species occurrence dataset which has contributed to national efforts at analyzing population-level trends at the national level. [@udell] At a regional level, the Northwest Bat Hub has also produced many publications influencing study design,[@reichert2021; @rodhouse2017] data analysis, [@irvine2018] and conservation management.[@rodhouse2019; @rodhouse2021]

Initial monitoring efforts began in Oregon but have since expanded to include partners in Washington (2018) and Idaho (2020). As a result, regional analyses have focused primarily on Oregon and Washington. The Northwest Bat Hub and it's partners now have three years of vetted summer acoustic monitoring data which include Idaho, and are uniquely positioned to expand analyses to encompass these data.

The NABat grid has been designed to account for species absence, and serves as the foundation for many analyses conducted using summer acoustic monitoring data. @reichert2021 The Maximum Entropy (MaxEnt) machine learning method leverages pseudo-absence (aka "presence-only") data to make prediction about species presence across a study area. MaxEnt does not provide a robust infrastructure for analyzing population trends across time, but does provide an approachable framework for producing region-wide range maps using the NABat summer acoustic monitoring data set.
...
library(tidyverse)
library(tidyterra)
library(here) #for relative pathnames
library(sf)
library(usmap)
library(terra)
library(gt)
...
Analyses included 14 species found commonly in the Pacific Northwest (@tbl-spec). *Tadirida brasiliensis* was excluded due to small sample size and its limited range in southern Oregon.

```{r}
#| label: tbl-spec
#| tbl-cap: "Common name, scientific name, and four-letter code for each of the bat species included in this analysis."
#| echo: false
#| eval: true
...
wbwg %>% 
  mutate("Common Name" = str_to_title(`COMMON NAME`),
         ...
         "Four Letter Code" = fourlettername) %>% 
  select("Common Name", "Scientific Name", "Four Letter Code") %>% 
  filter("Four Letter Code" != "tabr") %>% 
...
The following code has been serialized to fit the model for all species of interest.
