...
data.acoustic <- data.acoustic %>% 
  ...
  mutate(CONUS =
           case_when(project %in% c('Oregon','Washington') ~ SU,
                     TRUE ~ CONUS_10KM),
         GRTS = 
           case_when(project %in% c('Idaho','USFWS','USFS_Region04') ~ SU,
                     TRUE ~ GRTS_ID),
...
### 130595_SW3 has all noise files, internal clock battery died, time not reset
### 126901_SW3 and NW1 has all noise
...
for(i in 1:length(data.acoustic$Path)){
  if(str_detect(data.acoustic$Path[i], 'ZeroCrossing')){
    tmp1 <- read.csv(paste0('E:/NABat/Idaho/Processed/',year,'/KaleidoscopeOutput/', data.acoustic$GRTS[i], '_ZeroCrossing/id.csv'))
    ...
    data.acoustic$Path[i] <- paste0('E:/NABat/Idaho/Processed/2023/KaleidoscopeOutput/', data.acoustic$GRTS[i], '_ZeroCrossing')
  }else{
...
if(project == 'USFS_Region04'){
  CellTracker <- read_excel(paste0('C:/Users/',onid,'/Box/HERS_Working/Bats/Coordination/NABat_CellTracker_',year,'.xlsx')) %>% 
    select(CONUS_10KM,USFS_Unit)
  classifier <- read_excel(paste0('C:/Users/',onid,'/Box/HERS_BatAcousticFiles/FS R4 Call Analysis/Region4_BatSpeciesList.xlsx')) %>% 
    select(1:2)
  classifier <- CellTracker %>% 
    left_join(classifier,
              by = c('USFS_Unit' = 'Forest')) %>% 
    select('CONUS' = CONUS_10KM,Classifier)
}else{
  classifier <- read_csv(paste0('C:/Users/',onid,'/Box/HERS_Working/Bats/DataManager/Scripts/ConusGrtsClassifier.csv')) %>% 
    select('CONUS' = CONUS_10KM,Classifier)
}
...
data.acoustic <- data.acoustic %>% 
  mutate(Classifier = 
           case_when(Project == 'Idaho' ~ 'Eastern Washington',
                     Project == 'Nevada' ~ 'Great Basin',
                     TRUE ~ Classifier))
...
### Save VettingTracker ###
###########################
write_csv(data.acoustic, paste0('E:/NABat/',project, '/VettingTracker_','USFS_Region04_', format(Sys.Date(),'%Y%m%d'), '.csv'), na = '')
